...
type Note struct {
	...
	UserId        uint64 // leave empty on the client
...
func createNote(user User, title string, desc string, body string, tag string) uint64 {
	if title != "" {
		var chk_unique_title []Note
		db.Where("user_id = ?, title = ?", user.Id, title).Find(&chk_unique_title)
...
func do_create(user User, note Note) uint64 {
